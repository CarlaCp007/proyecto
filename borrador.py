...
class SoundManager:
    def __init__(self, music_path, sonido_img, mute_img, icon_position):
        ...
        # Cargar iconos de sonido
        try:
            self.sonido = pygame.image.load(sonido_img).convert_alpha()
            self.mute = pygame.image.load(mute_img).convert_alpha()
        except pygame.error as e:
            print(f"Error al cargar los iconos de sonido: {e}")
            pygame.quit()
            sys.exit()
    ...
    def increase_volume(self):
        ...
        nuevo_volumen = min(current_volume + VOLUME_STEP, MAX_VOLUME)
        pygame.mixer.music.set_volume(nuevo_volumen)

    def decrease_volume(self):
        current_volume = pygame.mixer.music.get_volume()
        nuevo_volumen = max(current_volume - VOLUME_STEP, MIN_VOLUME)
        pygame.mixer.music.set_volume(nuevo_volumen)
    ...
    def draw(self, pantalla):
        if pygame.mixer.music.get_volume() > 0:
            pantalla.blit(self.sonido, self.icon_position)
        else:
            pantalla.blit(self.mute, self.icon_position)
...
class Game:
    ...
    def run(self):
        ...
        while ejecuta:
            ...
            # Dibujar elementos de UI (icono de sonido/mute)
            self.sound_manager.draw(self.pantalla)
